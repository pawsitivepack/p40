...
exports.cancelBookedWalk = async (req, res) => {
	try {
		const { walkId } = req.params;
		const userId = req.user.id;

		const bookedWalk = await BookedWalks.findById(walkId);
		if (!bookedWalk) {
			return res.status(404).json({ message: "Booked walk not found." });
		}

		// Only allow the booking owner to cancel
		if (!bookedWalk.userId.equals(userId)) {
			return res.status(403).json({ message: "You are not authorized to cancel this booking." });
		}

		// Update ScheduledWalk: increment slots, remove user from walker array
		if (bookedWalk.walkId) {
			await ScheduledWalk.findByIdAndUpdate(bookedWalk.walkId, {
				$inc: { slots: bookedWalk.slots || 1 },
				$pull: { walker: userId },
			});
		}

		// Optional: Remove from user's dogsWalked if you’re tracking that
		await User.findByIdAndUpdate(userId, {
			$pull: { dogsWalked: walkId },
		});

		// Delete the booked walk
		await BookedWalks.findByIdAndDelete(walkId);

		return res.status(200).json({ message: "Booked walk cancelled successfully." });
	} catch (error) {
		console.error("Cancel Booked Walk Error:", error);
		return res.status(500).json({ message: "Failed to cancel the booked walk." });
	}
};