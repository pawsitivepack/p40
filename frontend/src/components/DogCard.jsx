...
function DogCard({ dog, onDelete, onEdit }) {
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`http://localhost:5001/dogs/${dog._id}`);
      ...
      console.log(`${dog._id} You are trying to delete this dog named ${dog.name}`);
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      <div className="p-4 text-center">
        ...
        <p className="text-gray-600"><strong>Category:</strong> {dog.category}</p>
