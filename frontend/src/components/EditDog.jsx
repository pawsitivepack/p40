...
const EditDog = ({ setFormVisible, setDogs, dog }) => {
    ...
    const handleUpdateDog = async (e) => {
        ...
        try {
            const response = await axios.put(`http://localhost:5001/dogs/${dog._id}`, updatedDog);
            ...
            console.error('Error updating dog:', error.response ? error.response.data : error.message);
